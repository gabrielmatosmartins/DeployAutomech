...
export function OficinaScreen() {
    ...
    async function retornarVeiculos(id){
      const response = await fetch(`http://localhost:8080/Cliente/${id}/Veiculo`, { method: 'GET' })
      var data = await response.json();
    ...
    async function retornarUmVeiculo(){
      ...
      const response = await fetch(`http://localhost:8080/Veiculo/findById/${idVeiculo}`, { method: 'GET' })
      var data = await response.json();
      return data;
    }

    async function fazerOrcamento(id){
      let url = `http://localhost:8080/Orcamentos/Create`
